...
echo 'CopyrightⒸ2001-2018.Bruce_Guenther_Baumgart.Software_License:GPL-v3.'
if [ $(uname -m) == 'x86_64' ];then M=-m64;else M='';fi
echo gcc -g $M -Wall -Werror -o $BIN/$NAME $HOME/${NAME}-2018.c $HOME/md5.c
     gcc -g $M -Wall -Werror -o $BIN/$NAME $HOME/${NAME}-2018.c $HOME/md5.c
return
#endif
 ...
 * 2018 notes
 * ======================================================================================= 
# Setup logical links from the root directory into /large/
# disk work areas, which I use to mount on loop partitions for ease of delete,
# 300 GBytes is adquate for the large container for 2018 undart

	mkdir -p /large/2018/{data7/sn,data8/sn}
	chown bgb:bgb -R /large/2018

# Optional, but convenient, are logical links from root into the /large buckets as follows
        cd /
        ln -s /large/2018/data7  /data7      # this is for the 7-bit Stanford ASCII as utf8 text
        ln -s /large/2018/data8  /data8      # 8 bytes per PDP10 word

# The original DART TAR data in a single flat file
# Here is the raw data as I received it:
# The MD5 hash values before and after compression are:  
# a33c8ff234f0e106e2e99e219f3b7b45 dart_records_from_229_reels.tar
# ecc815253d76dffc615e311bfbb4c090 dart_records_from_229_reels.tar.lzma

# Unpack the compressed DART records from the 229 reels of tape ... 17m34s
# yielding 41_594 dart records
        cd /large/2018/dart
        time tar -xf /large/lzma/dart_records_from_229_reels.tar.lzma
...
# RESET or CLEAR OFF log and csv directories
        mv /large/log{,.prev}
        mv /large/log{,.prev}
   # OR
        rmdir -R /large/log /large/csv
   # Then
        mkdir    /large/log /large/csv

# Full run, undart. Single process. Took 90 minutes on 20 August 2018.
        cd /dartrecords
        # ASSERT readdir .|wc -l is 41594
        time undart p*
   # OR
        t1;time undart p* 1>/large/log/stdout 2>/large/log/stderr;t2
   # which took 100 minutes and 170 Gigabytes (32,53,85) for data (7,8,13).

# 2018 install Maria DB ( or MYSQL )
        apt-get install mariadb-server
        mysql_secure_installation
        mysql -h localhost -u root mysql -p PASSWORD_FOR_DB_ROOT

# 2018 create saildart database
        create user 'bgb'@'localhost' identified by 'foo99baz';
        drop database if exists saildart;
...
# create static web site for /large
        ln -s /large/static           /static
        ln -s /large/static/html      /html
        ln -s /large/content          /content
 * =======================================================================================
 * copyright:(c)Ⓒ2001-2018 Bruce Guenther Baumgart
 ...
 * globals 
 */
        char *saildartroot = "/large/2018"; /* was "/data/YEAR" */
        FILE *stdlog;
        FILE *stdcsv;
        FILE *snhash;
...
// 2018:
// OMIT without NUL, RETURN, ALT and RUBOUT ! from UTF8 data7 text files
//
unsigned short sail_unicode[128]={
         ...
         0x03bb, // λ lambda
         011, //      TAB
         012, //      LF as white space
         0,   // omit VT 013
         014, //      FF
         0,   // omit CR 015 as white space
         0x221e, // ∞ infinity
         ...
         0x7c,  // | vertical bar
         0,     // 0xa7 § Stanford § Altmode is a keystroke character glyph NOT a shift key.
         0x7d,  // } right curly bracket ASCII octal 0175, but SAIL code 0176 !
         0      // omit RUBOUT from text files 0x232b Backspace unicode BS backspace glyph ␈ u2408  glyph ⌫ u232b
};

/*
...
output_data7_file( int wrdcnt ){
  ...
  errno = 0;
  // NOT text file. So no need for UTF8 blob.
  if(!( filetype=='C' || filetype=='T' )){
    return;
  }
...
long check_sn(char *hash){
  ...
  f= hsearch(e,FIND);
  if(!f){
        fprintf(stderr,"check_sn /%s/ NOT found\n",hash);
        return 0;
  }
...
#endif
// The snhash8 is the "Accession-Numbering" for this "Digital-Archive".
void load_old_snhash8(){
  ENTRY e,*f;
  FILE *old;
  char *old_snhash8="/large/sn-hash8-accession";
  long sn,cnt=0;
  // char altkey[24];
  errno = 0;
...

// mark when 'seen' table
unsigned char snhash8[999999];
// 2018: this function computes buffer data8 HASH and sets global serial_number.
int
output_data5_file(int wrdcnt){
  char *p, *q;
  int i;
  
  // The MD5 hash of the data8 buffer is the lookup key for the sn, ACCESSION NUMBER.
  md5_init_ctx( &ctx );
  md5_process_bytes( obuf8, wrdcnt*8, &ctx );
  md5_finish_ctx( &ctx, data8_MD5_digest );
  p = (char *)data8_MD5_digest;
  ...
  serial_number = check_sn( data8_MD5_hash ); // For 2018, the data8 hash is the accession key
  if( !serial_number ){
    fprintf(stderr,"NO accession number for hash8='%s'\n",data8_MD5_hash);
    return 0; // NO accession number for this blob. Ignore it.
  }
  if( snhash8[serial_number] ){
    return 0; // Already did this one.
  }
  snhash8[serial_number] = 1; // mark TRUE.
  return 1;
}

void
 ...
 */
  ...
  /*
   * Test for crypt MAGIC marker.
   */
  if ( obuf8[0]== 0714153265353LL ){
    filetype = 'Y';
  }
}

